...
class CalculationService {
  async evaluateExpression(expression, userId) {
    try {
      ...
      this.saveCalculationWithFailover(userId, expression, result)
        .catch(err => console.error('All save attempts failed:', err));
  ...
  async saveCalculationWithFailover(userId, expression, result) {
    try {
      ...
      console.warn('Primary API save failed, trying backup...', primaryError);
      
      try {
        await this.saveToAPI(BACKUP_API_BASE, userId, expression, result);
      } catch (backupError) {
