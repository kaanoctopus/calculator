...
export async function fetchWithFallback(endpoint, options, isAuthEndpoint = false) {
  ...
  const now = Date.now();
  const isInFailover = primaryAPIFailedAt && (now - primaryAPIFailedAt < FAILOVER_DURATION);
  ...
  if (!isInFailover) {
    try {
      ...
      console.warn('Primary API failed, entering failover mode', primaryError);
      primaryAPIFailedAt = now;
    }
  } else {
    console.log(`In failover mode (${Math.round((FAILOVER_DURATION - (now - primaryAPIFailedAt))/1000)}s remaining)`);
  }
